...
class EmailService {
  ...
  init() {
    this.senderEmail = process.env.FROM_EMAIL || null;
    ...
    if (this.apiKey) {
      ...
      console.warn("SendGrid API key not found. EmailService not initialized.");
    }
  }

  async sendEmail(toEmail, subject, htmlBody) {
    if (!this.initialized) {
      console.error("EmailService no inicializado");
      return false;
    }
    ...
    try {
      ...
      console.error("[EmailService] Error:", err?.response?.body || err.message || err);
      return false;
    }
  }
                    ...
                    <table role="presentation" style="max-width: 600px; margin: 0 auto; background-color: #ffffff; border-radius: 12px; overflow: hidden; box-shadow: 0 4px 6px rgba(0,0,0,0.1);">
                        ...
                        <tr>
                            <td style="padding: 40px 30px;">
                                ...
                                <div style="background-color: #e8f5f4; border-left: 4px solid #5BA8A0; padding: 15px; border-radius: 8px; margin: 0 0 25px;">
                                    <p style="margin: 0; color: #4A9B94; font-size: 14px; font-weight: 600;">
